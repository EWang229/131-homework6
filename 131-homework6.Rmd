...
```{r}
pokemon_continuous <- pokemon[, c(6, 7, 8, 9, 10, 11)]
pokemon_cor <- cor(pokemon_continuous)
corrplot(pokemon_cor, method = "number", order = "alphabet", type = "lower")
```

I kept only the variables that were continuous in nature, so I did not include legendary or generation. I opted not to include the variables that were not in our recipe as well.

What relationships, if any, do you notice? Do these relationships make sense to you?

I notice that attack and defense, sp_atk and attack, speed and attack, sp_def and defense, hp and sp_def, sp_atk and sp_def, and speed and sp_atk were moderately (positively) correlated. I agree with most of the relationships, except for attack and defense. I would have thought that attack and defense would be negatively correlated to balance out the pokemon stats, but their positive association is quite strong! 
...
autoplot(tune_res)
```
I observe that less trees tends to yield worse results, the results get better as the mtry values increase, and that a higher minimal node size tends to get better results. Though it really depends on the combination of value for the three hyperparameters.
